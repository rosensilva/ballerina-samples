...
This guide walks you through the process of adding timeouts and retry patterns to potentially-busy remote backend calls. Timeout pattern will automatically cut off the remote services if it fails to respond before the deadline. Well-placed timeouts provide isolation from network faliures. Retry pattern calls the remote backed services again and again until it gets response or retry count reached. Retry pattern is used to get response from remote service even it fails several times.
...
The `product_search` is the service that handles the client requests. product_search incorparate resiliency pattern like timeouts and retry when calling potentially-busy remote eCommerce backend.  

The `ecommerce_backend` is an independent web service that accepts orders via HTTP GET method and sends the item details back to the client.
### Develop the Ballerina services

#### product_serach_service.bal
The `product_serach_service.bal` is the service which incorparates the retry and timeout resiliency pattens. You need to pass the remote endpoint timeout and retry configurations when you define the HTTP client endpoint. 
...
The argument `endpointTimeout` refers to the remote HTTP client timeout in milliseconds and `retryConfig` refers to retry configuration. There are two parameters in the the retry configuration, `count` refers to the number of retires and the `interval` refers to the time interval between two consecative retries. The `eCommerceEndpoint` is the reference for the HTTP endpoint of eCommerce backend. Whenever you call that remote HTTP endpoint it uses the retry and timeout resillency patterns.
...
Please find the implementation of the inventory management service in `ballerina-guides/resiliency-timeouts/guides/ecommerce_backend/ecommerce_backend_service.bal`
...
2. Then invoke the product_search by quering an item via HTTP GET method. 
   ``` bash
    curl 0.0.0.0:9090/products/search?item=TV
   ```
   The order service should finally respond after several timeouts and retires with the following json message,
   ```json
